describe('Navigator responder behavior/interface validation', function () {
    ...
    describe('Retrieving interface inheritance chain', function () {
        it('Returns an empty array when no interface argument is supplied', function () {
            const interfaces = navigatorjs.NavigationResponderBehaviors.getInterfaceInheritanceChain();
        ...
        it('Returns an array of one element that is equal to the input when just a toplevel (IHasStateInitialization) interface is defined', function () {
            const interfaces = navigatorjs.NavigationResponderBehaviors.getInterfaceInheritanceChain('IHasStateInitialization');
        ...
        it('Returns an empty array when an invalid interface argument is supplied', function () {
            const interfaces = navigatorjs.NavigationResponderBehaviors.getInterfaceInheritanceChain('IHasInvalidStateInitialization');
            expect(interfaces).toEqual([]);
        });

        it('Returns an empty array when a method (getInterfaceInheritanceChain) as the interface argument is supplied', function () {
            const interfaces = navigatorjs.NavigationResponderBehaviors.getInterfaceInheritanceChain('getInterfaceInheritanceChain');
            expect(interfaces).toEqual([]);
        });

        it('Returns simple inheritance chain', function () {
            const interfaces = navigatorjs.NavigationResponderBehaviors.getInterfaceInheritanceChain('IHasStateValidationOptional');
        ...
        it('Returns an advanced inheritance chain and filters out duplicate interfaces', function () {
            const interfaces = navigatorjs.NavigationResponderBehaviors.getInterfaceInheritanceChain('IHasStateValidationOptionalAsync');
    ...
    describe('Retrieving methods by interface', function () {
        it('Returns no methods for an invalid interface', function () {
            let methods = navigatorjs.NavigationResponderBehaviors.getInterfaceMethods([]);
        ...
        it('Returns a list of unique methods within a simple inheritance chain', function () {
            const methods = navigatorjs.NavigationResponderBehaviors.getInterfaceMethods(['IHasStateInitialization']);
        ...
        it('Returns a list of unique methods within an advanced inheritance chain', function () {
            const methods = navigatorjs.NavigationResponderBehaviors.getInterfaceMethods(['IHasStateValidationOptionalAsync']);
        ...
        it('Returns a list of unique methods within a list of multiple interfaces and advanced inheritance chain', function () {
            const methods = navigatorjs.NavigationResponderBehaviors.getInterfaceMethods(['IHasStateInitialization', 'IHasStateValidationOptionalAsync']);
    ...
    describe('Validate interface implementation', function () {
        ...
        it('Checks inherited behaviours', function () {
            const object = {
                ...
                validate(truncatedState, fullState) {}, // IHasStateValidation, IHasStateValidationAsync, IHasStateValidationOptional, IHasStateValidationOptionalAsync, IHasStateRedirection
                willValidate(truncatedState, fullState) { /* return bool*/ } // IHasStateValidationOptional, IHasStateValidationOptionalAsync
            };
        ...
        it('Has implemented all behaviors', function () {
            const object = {
                ...
                initializeByNavigator() {}, // IHasStateInitialization
                validate(truncatedState, fullState) { /* return bool*/ }, // IHasStateValidation, IHasStateValidationAsync, IHasStateValidationOptional, IHasStateValidationOptionalAsync, IHasStateRedirection
                prepareValidation(truncatedState, fullState, callOnPrepared) {}, // IHasStateValidationAsync, IHasStateValidationOptionalAsync
                willValidate(truncatedState, fullState) { /* return bool*/ }, // IHasStateValidationOptional, IHasStateValidationOptionalAsync
                redirect(truncatedState, fullState) { /* return NavigationState*/ }, // IHasStateRedirection
                willSwapToState(truncatedState, fullState) { /* return bool*/ }, // IHasStateSwap
                swapOut(callOnSwapOutComplete) {}, // IHasStateSwap
                swapIn(truncatedState, fullState) {}, // IHasStateSwap
                transitionIn(callOnComplete) {}, // IHasStateTransition
                transitionOut(callOnComplete) {}, // IHasStateTransition
                updateState(truncatedState, fullState) {} // IHasStateUpdate
            };
        ...
        describe('redirection', function () {
            beforeEach(function () {
                ...
                Responder.prototype = {
                    ...
                    redirect(truncatedState, fullState) {
            ...
            it("doens't throw an error when we redirect to a substate, as long as the validate doesn't return false for the substate", function () {
                responder.validate = function (truncatedState, fullState) {
                    return !fullState.equals('home');
                };
                responder.redirect = function (truncatedState, fullState) {
                    return new navigatorjs.NavigationState('home/test');
                };
                njs.add(responder, 'home');
                njs.add({}, 'home/test');
                njs.start('/');
            ...
            it('can chain redirects', function () {
                const contact = new Responder();
                contact.redirect = function (truncatedState, fullState) {
                    return new navigatorjs.NavigationState('about');
                };
        ...
        describe('swapping', function () {
            beforeEach(function () {
                ...
                Responder.prototype = {
                    ...
                    willSwapToState(truncatedState, fullState) {
                        this.willSwapToStateCount++;
                        return this.willSwapToStateReturnValue;
                    },

                    swapIn(truncatedState, fullState) {
                        this.swapInCount++;
                    },

                    swapOut(callOnSwapOutComplete) {
                        this.swapOutCount++;
            ...
            describe('synchronous', function () {
                describe('IHasStateValidation', function () {
                    ...
                    it('first validates both the parent and child states', function () {
                        const childResponder = new Responder();
                        njs.add(responder, 'validation');
                    ...
                    it("doesn't validate the child state if the parent state did not validate", function () {
                        ...
                        const childResponder = new Responder('child');
                        njs.add(responder, 'validation');
                        njs.add(childResponder, 'validation/test');
                        njs.start('/');
                        njs.request('validation/test');
            ...
            // TODO this is probably an issue. Should we order the states we loop through internally?
                ...
                describe('IHasStateValidationOptional', function () {
                    ...
                    it("does not execute the validate method if the responder returns that we don't need to run custom validation", function () {
                        responder.willValidate = function () {
                            return false;
                        };
                        njs.add(responder, 'validation');
                        njs.start('/');
                ...
                describe('IHasStateValidationAsync', function () {
                    beforeEach(function () {
                        ...
                        Responder.prototype = {
                            ...
                            prepareValidation(truncatedState, fullState, callOnPrepared) {
                    ...
                    it('stacks validation calls of substates that has to be validated', function () {
                        const childValidateResponder = new Responder();
                        njs.add(responder, 'validation');
                    ...
                    it("Doesn't allow to navigate when the child responder's validate method returns false", function () {
                        const childValidateResponder = new Responder();
                        childValidateResponder.validate = function () {
                            return false;
                        };
                        njs.add(responder, 'validation');
                        njs.add(childValidateResponder, 'validation/test');
